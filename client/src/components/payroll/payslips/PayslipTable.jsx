...
const PayslipTable = ({ data = [], loading = false, error = null, onEdit, onDelete }) => {
  if (loading) return <Loading message="Loading payslips..." />;
  if (error)   return <ErrorMessage message={error} />;
  if (!data.length) return <EmptyState title="No Payslips Found" description="No records to display." />;
